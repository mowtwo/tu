...
The plugin's `load` hook reads the `.tu` file, calls `compile()` from `@tu/compiler`, and returns the result. Vite handles the rest like any JS module — including HMR via full module re-import on save.
