...
- **Diagnostics.** Every `.tu` open / change / save runs through:
  1. `compileToTS()` from `@tu-lang/compiler` (preserves lambda param types so tsserver can infer everything else)
...
- Token-level diagnostic ranges. Right now the M2 V1 source map is per-top-level-statement, so a diagnostic shows up at the start of the offending `let` / `import` line, not pointed at the exact token. Fixing this requires a finer source-map emit pass in `@tu-lang/compiler`.
...
```ts
import { checkTuSource, checkTuFile, type TuDiagnostic } from '@tu-lang/lsp'
