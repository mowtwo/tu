...
| LSP V2: hover, completion, goto-definition, rename | M3 V1 | post-M3 | M3 V1 ships diagnostics only via the TypeScript Compiler API. Richer IDE features need either the Volar framework or a deeper tsserver integration. M3.2 lays the groundwork (every AST node now carries a source range; codegen emits a TokenMapping per leaf). |
