...
export let Page = () => div {
  markdown {
    ...
    | Suspense + streaming SSR + resumability | M1.7 | M6.11+ | M6.11 (#60) shipped `renderToStringAsync` + `renderPageAsync` and wired tu-shu's `loadTuPage` through the async path — Tu `async () => …` components now SSR correctly. Sync `renderToString` throws a typed `TuRenderError` on Promise children (was: silently `[object Promise]`). Still open: `<Suspense>` boundary primitive (#61), streaming SSR via ReadableStream + per-boundary `<template>` flush (#62), and Qwik-style resumability (post-M6.11). See `docs/SSR-ASYNC-DESIGN.md` for the design. |
