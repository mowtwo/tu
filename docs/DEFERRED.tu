...
export let Page = () => div {
  markdown {
    ...
    | Remaining JS expression operators | M5.10 | TBD | M5.10 added `\|\|`, `&&`, `??`, `?.`, `?.()`, `?.[]`, `obj[k]`, prefix `!` / `-` / `+`, postfix `!` (TS non-null assertion), parenthesized expressions, and TS-style optional params (`name?: T`). Still missing: ternary `?:` (use Tu's `if` expression instead), template literals `` `${…}` ``, spread `...`, prefix/postfix `++`/`--`, compound assignment (`+=`, `-=`, `\|\|=`, etc.), bitwise (`&` / `\|` / `^` / `~` / `<<` / `>>`), `**` exponent, `instanceof` / `typeof` / `void` / `delete`. Add piecemeal as user code demands. |
