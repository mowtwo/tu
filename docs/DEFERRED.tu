...
export let Page = () => div {
  markdown {
    ...
    | Member compound assignment | M6.5 | TBD | `props.value ||= "default"` / `arr[i] += 1`. Tu's AssignExpr currently locks the target to a string ident so cell semantics can fire; broadening it to MemberExpr / IndexExpr is invasive. Workaround today: `obj.prop = obj.prop \|\| "default"` (works) or `obj = { ...obj, prop: x }` for immutable spreads. |
