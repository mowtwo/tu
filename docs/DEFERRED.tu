// Generated from docs/DEFERRED.md — Tu-native docs page.
...
export let Page = () => div {
  markdown {
    ...
    | Static-HTML optimization (skip h() for non-reactive subtrees) | M1.0 | VERIFY | The runtime ships a `$static` tag sentinel claiming M6.0 landed (see `packages/runtime/src/index.ts` `STATIC_TAG`). User-flagged 2026-05-02 that not every advertised feature has example/playground proof. Audit during the LSP DX milestone — confirm codegen actually emits `$static` for non-reactive subtrees, then close this row OR finish the missing half. |
    ...
    | Remaining JS expression operators | M5.10 | TBD | M5.10 + M6.5 covered `\|\|`, `&&`, `??`, `?.`, `?.()`, `?.[]`, `obj[k]`, prefix `!` / `-` / `+` / `++` / `--`, postfix `!` (TS non-null) / `++` / `--`, ternary `cond ? a : b`, `new`, compound assign (`+=`, `-=`, `\|\|=`, etc. — for cell idents), spread `...` (call args + array + object), template literals `` `${…}` ``, parens, TS optional params. Still missing: regex literals `/…/flags`, bitwise (`&` / `\|` / `^` / `~` / `<<` / `>>`), `**` exponent, `instanceof` / `void` / `delete`. Add piecemeal as user code demands. |
    ...
    | Verify Style ↔ JS state interop status | M1.8 | post-LSP DX | User said 2026-05-02 they thought this landed in the earliest Tu version but cannot find any example or playground demonstrating it. Audit current style-block codegen first; either ship proof examples + playground demo OR design from scratch. See `memory/project_style_state_interop.md` for the original sketches (CSS custom properties auto-bound to Tu cells, `@bind` rule, `:state(...)` selectors). |
